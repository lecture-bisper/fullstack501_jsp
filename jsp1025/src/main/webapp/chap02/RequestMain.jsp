...
<%--  form 태그 : html 태그 중에서 사용자가 입력한 데이터를 지정한 주소로 전달 --%>
<%--  input, textarea, select 태그에 저장된 데이터만 전달, form 태그의 자식 태그로 지정된 태그의 데이터만 전달 --%>
<%--  input, textarea, select 태그의 name 속성값이 서버에서 request 내장 객체에서 데이터를 구분하는 이름이 됨 --%>
